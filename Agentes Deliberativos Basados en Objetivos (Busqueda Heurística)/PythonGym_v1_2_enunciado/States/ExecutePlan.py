...
class ExecutePlan(State):
    ...
    def Update(self, perception, map, agent):
        ...
        if  len(plan) <= 1 and (goal.value == AgentConsts.PLAYER or goal.value == AgentConsts.COMMAND_CENTER): 
            ...
            shot = self.ShouldFire(perception, map, agent, move) and perception[AgentConsts.CAN_FIRE] == 1
    ...
    def ShouldFire(self, perception, map, agent, move):
        mapSize = agent.problem.ySize
        xW = perception[AgentConsts.AGENT_X]
        yW = perception[AgentConsts.AGENT_Y]
        x, y = BCProblem.WorldToMapCoord(xW, yW, mapSize)
        x = int(x)
        y = int(y)


        def cell(x, y):
            if 0 <= x < agent.problem.xSize and 0 <= y < agent.problem.ySize:
                return agent.problem.map[x][y]
            return AgentConsts.UNBREAKABLE  # fuera de mapa = muro
        ...
        if move == AgentConsts.MOVE_UP:
            ...
            return False  # no se mueve => no dispara

        return (front == AgentConsts.BRICK or
                front == AgentConsts.SEMI_BREKABLE or
                left == AgentConsts.BRICK or
                left == AgentConsts.SEMI_BREKABLE or
                right == AgentConsts.BRICK or
                right == AgentConsts.SEMI_BREKABLE or
                front == AgentConsts.COMMAND_CENTER or
                cell(x, y) == AgentConsts.SEMI_BREKABLE or
                cell(x, y) == AgentConsts.BRICK)
