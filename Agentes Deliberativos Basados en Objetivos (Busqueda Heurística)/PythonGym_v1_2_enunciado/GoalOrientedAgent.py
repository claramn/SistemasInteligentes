...
class GoalOrientedAgent(BaseAgent):
    def __init__(self, id, name):
        ...
        self.mapSize = (15,15) #tamaño del mapa (x,y)
    ...
    def _CreatePlan(self,perception,map):
        ...
        if self.goalMonitor != None:
            ...
            goalActual = self.goalMonitor.SelectGoal(perception,map,self)  
            print("Objetivo seleccionado:", goalActual)     #seleccionamos meta mas adecuada
            self.problem.SetGoal(goalActual)
            
            nodoIni = self._CreateInitialNode(perception) #creamos el nodo inicial

           # self.problem.SetStart(nodoIni) #le pasamos el nodo inicial al problema
            self.problem.setInitial(nodoIni)
            self.problem.InitMap(map) #inicializamos el mapa del problema
            plan = self.aStar.GetPlan() #calculamos el plan
            GoalOrientedAgent.ShowPlan(plan)  # Muestra el plan por consola

            return plan
            #return self._OptimizePlan(plan) #optimizamos el plan y lo devolvemos

        return []
    """
    def _OptimizePlan(self, plan):
        if len(plan) < 2:
            return plan
        planOptimo = [plan[0]]  
        direccionAct = plan[0].action

        for i in range(1, len(plan)):
            if plan[i].action != direccionAct:
                planOptimo.append(plan[i])
                direccionAct = plan[i].action
        return planOptimo
    """

    def _Replan(self, perception,map):
        self.problem.InitMap(map) #inicializamos el mapa del problema
        self.plan = self._CreatePlan(perception, map) #calculamos el plan

        if hasattr(self.stateMachine.curentState, 'SetPlan'):
            self.stateMachine.curentState.SetPlan(self.plan)
    ...
    def _CreateCommandCenterGoal(self, perception):
    ...
    def InitAgent(self,perception,map):
        ...
        self.problem = BCProblem(None,None,self.mapSize[0],self.mapSize[1])

        self.problem.InitMap(map) #inicializamos el mapa del problema
        goal1CommanCenter = self._CreateCommandCenterGoal(perception) #creamos el nodo inicial
        goal2Life = self._CreateLifeGoal(perception)
        goal3Player = self._CreatePlayerGoal(perception)
        ...
        if hasattr(self.stateMachine.states["ExecutePlan"], 'SetPlan'):
            self.stateMachine.states["ExecutePlan"].SetPlan(self.plan)
